#!/usr/bin/env python3
# strategies_zodiac.py - 生肖预测策略（完整版）
...
def _compute_zodiac_score_decay(rows, recent_window=30, special_boost=3.0, decay=0.95):
    scores = {z: 0.0 for z in ZODIAC_MAP}
    weight = 1.0
    for i, r in enumerate(rows[:recent_window]):
        for n in _row_numbers(r):
            scores[get_zodiac_by_number(n)] += weight
        scores[get_zodiac_by_number(_row_special(r))] += weight * special_boost
        weight *= decay
    return scores
...
def predict_strong_three_improved(rows, window, use_decay=True, xgb_weight=0.0, xgb_model=None):
    if use_decay:
        scores = _compute_zodiac_score_decay(rows, recent_window=window, special_boost=3.0, decay=0.95)
    else:
