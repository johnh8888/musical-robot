...
def stable_score(history):
    ...
    for idx, r in enumerate(reversed(recent[-12:])):
        for n in r["numbers"] + [r["special_number"]]:
            z = get_zodiac(n)
            score[z] += 0.58 * decay(idx)

    for idx, r in enumerate(reversed(recent[-50:])):
        for n in r["numbers"] + [r["special_number"]]:
            z = get_zodiac(n)
            score[z] += 0.17 * decay(idx)

    sp_list = [r["special_number"] for r in recent[-28:]]
    ...
    for z in ZODIAC_LIST:
        ...
        if (is_odd(sample) and odd_ratio > 0.5) or (not is_odd(sample) and odd_ratio <= 0.5):
            bonus += 0.15
        score[z] += bonus + freq.get(z, 0) * 0.09

    om = omission_map(history)
    for z in ZODIAC_LIST:
        score[z] += min(om[z] * 0.10, 1.8) * 0.27
