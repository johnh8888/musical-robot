...
def stable_score(history):
    ...
    # 波色趋势奖励
    recent_colors = [get_color(r["special_number"]) for r in recent[-10:]]
    color_count = Counter(recent_colors)
    dominant = color_count.most_common(1)[0][0] if color_count else "红"

    for z in ZODIAC_LIST:
        sample = ZODIAC_MAP[z][0]
        if get_color(sample) == dominant:
            score[z] += 0.25

    # 遗漏补偿
    om = omission_map(history)
    for z in ZODIAC_LIST:
        score[z] += min(om[z] * 0.048, 0.60) * 0.13
...
def backtest(rows, lookback=120):
    ...
    print("\n===== 优化后一二三肖回测 =====")
    print(f"测试期数: {total}")
    print(f"一肖命中: {s_hit/total:.1%} | 最大连空 {s_max}")
    print(f"二肖命中: {t_hit/total:.1%} | 最大连空 {t_max}")
    print(f"三肖命中: {th_hit/total:.1%} | 最大连空 {th_max}")
